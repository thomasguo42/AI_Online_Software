...
def main(video_path, output_path, reid_model_path):
    ...
    for i, det in enumerate(selected_detections):
        ...
        center_x = int(np.round((det['box'][0] + det['box'][2]) / 2))
        center_y = int(np.round((det['box'][1] + det['box'][3]) / 2))
    ...
    for frame_idx, frame in enumerate(tqdm(frames[1:], desc="Processing frames"), 1):
        ...
        if tracks and detections:
            ...
            for i, j in zip(row_ind, col_ind):
                if cost_matrix[i, j] < (1 - similarity_threshold):
                    det = detections[i]
                    track = tracks[j]
                    tracker.samcontroler.sam_controler.reset_image()
                    ...
                    binary_mask = (mask > 0).astype(np.uint8) * track['label']
                    track['embedding'] = det['embedding']
                    track['box'] = det['box']
                    track['mask'] = binary_mask
                    track['logit'] = logit

        # Paint frame with masks and draw bounding boxes
        for track in tracks:
...
if __name__ == "__main__":
    video_path = "4408.mp4"  # Replace with your video path
