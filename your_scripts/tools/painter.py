...
def vis_add_mask(input_image, mask, color, alpha=0.5):
    """Add mask to the input image with the given color and alpha."""
    # Ensure color index is within bounds of color_list
    color = np.array(color_list[color % len(color_list)])
    
    # Apply the mask to the image (existing logic)
    mask = mask.astype(bool)
    for c in range(3):
        input_image[:, :, c] = np.where(
            mask,
            input_image[:, :, c] * (1 - alpha) + alpha * color[c],
            input_image[:, :, c]
        )
    return input_image
...
def mask_painter(input_image, input_mask, mask_color=5, mask_alpha=0.7, contour_color=1, contour_width=3):
    ...
    mask = np.clip(input_mask, 0, 1).astype(np.uint8)  # Ensure mask is uint8
    contour_radius = (contour_width - 1) // 2
